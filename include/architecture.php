...
<dd class="col-md-8"><pre class="bg-light text-dark"><code>image=AllocateImage(image_info);
...
<dd class="col-md-8"><pre class="bg-light text-dark"><code>(void) strcpy(image_info-&gt;filename,"image.jpg"):
image=ReadImage(image_info,exception);
if (image == (Image *) NULL)
  { /* an exception was thrown */ }
</code></pre></ul></dd>
<dt class="col-md-8">Create an image from a memory based image:</dt><br/>
<dd class="col-md-8"><pre class="bg-light text-dark"><code>image=BlobToImage(blob_info,blob,extent,exception);
if (image == (Image *) NULL)
  { /* an exception was thrown */ }
</code></pre></ul></dd>
</dl>
...
<ul><pre class="pre-scrollable bg-light text-dark"><code>const PixelPacket
  *p;
...
<ul><pre class="bg-light text-dark"><code>const IndexPacket
  *indexes;
...
<ul><pre class="bg-light text-dark"><code>p=GetVirtualPixels(source,-3,-3,source-&gt;columns+3,6,exception);
</code></pre></ul>
...
<ul><pre class="bg-light text-dark"><code>&lt;policymap>
...
<ul><pre class="bg-light text-dark"><samp>$ convert -debug cache logo: -sharpen 3x2 null:
...
<ul><pre class="bg-light text-dark"><code>convert -distribute-cache 6668 &amp;  // start on 192.168.100.50
...
<ul><pre class="pre-scrollable bg-light text-dark"><code>CacheView
  *view_1,
  *view_2;
...
<ul><pre class="bg-light text-dark"><code>convert -limit memory 2GB -limit map 4GB -define registry:temporary-path=/data/tmp ...
</code></pre></ul>
...
<ul><pre class="bg-light text-dark"><code>convert image.tif image.mpc
...
<ul><pre class="pre-scrollable bg-light text-dark"><code>static size_t StreamPixels(const Image *image,const void *pixels,const size_t columns)
{
...
<ul><pre class="bg-light text-dark"><code>(void) printf("image width: %lu, height: %lu\n",image-&gt;columns,image-&gt;rows);
</code></pre></ul>
...
<ul><pre class="bg-light text-dark"><code>const char
  *comment;
...
<ul><pre class="bg-light text-dark"><code>StringInfo
  *profile;
...
<ul><pre class="bg-light text-dark"><code>convert logo: -resize 250000x250000 logo.miff
</code></pre></ul>
...
<ul><pre class="bg-light text-dark"><code>convert -define registry:temporary-path=/data/tmp logo:  \ <br/>     -resize 250000x250000 logo.miff
</code></pre></ul>
...
<ul><pre class="bg-light text-dark"><code>convert -define registry:temporary-path=/data/tmp -limit memory 16mb \
  logo: -resize 250000x250000 logo.miff
</code></pre></ul>

<p>Here we force all image pixels to disk:</p>
<ul><pre class="bg-light text-dark"><code>convert -define registry:temporary-path=/data/tmp -limit area 0 \
  logo: -resize 250000x250000 logo.miff
</code></pre></ul>
...
<ul><pre class="bg-light text-dark"><code>convert -monitor -limit memory 2GiB -limit map 4GiB -define registry:temporary-path=/data/tmp \
  logo: -resize 250000x250000 logo.miff
</code></pre></ul>
...
<ul><pre class="pre-scrollable bg-light text-dark"><code>#include "Magick++.h"
#include &lt;assert.h&gt;
#include "omp.h"
...
<ul><pre class="bg-light text-dark"><code>&lt;policy domain="resource" name="thread" value="2"/>
</code></pre></ul>
...
<ul><pre class="bg-light text-dark"><code>export MAGICK_THREAD_LIMIT=12
</code></pre></ul>
...
<ul><pre class="bg-light text-dark"><samp>$ convert -bench 10 model.png -sharpen 5x2 null:
...
<ul><pre class="bg-light text-dark"><samp>-> identify -version
...
<ul><pre class="bg-light text-dark"><code>convert image.png -convolve '-1, -1, -1, -1, 9, -1, -1, -1, -1' convolve.png
</code></pre></ul>
...
<ul><pre class="pre-scrollable bg-light text-dark"><code>static inline long ClampToCanvas(const long offset,const ulong range)
{
  if (offset &lt; 0L)
    return(0L);
  if (offset >= range)
...
<ul><pre class="bg-light text-dark"><code>convert logo: logo.mgk
display logo.mgk
</code></pre></ul>
...
<ul><pre class="bg-light text-dark"><samp>convert logo: -process \"analyze\" -verbose info:
Image: logo:
