...
<pre class="bg-light"><code>tar xvzf ImageMagick.tar.gz
</code></pre>
...
<pre class="bg-light"><code>configure CC=c99 CFLAGS=-O2 LDFLAGS='-L/usr/local/lib -R/usr/local/lib' LIBS=-lposix
</code></pre>
...
<table class="table table-sm table-hover">
  ...
  <br /><br />
<pre class="bg-light">Quantum Depth     Valid Range (Decimal)   Valid Range (Hex)
    8             0-255                   00-FF
  ...
  <br /><br />
<pre class="bg-light">Quantum Depth   Virtual Memory
     8               3MB
    16               8MB
    32              15MB
</pre></td>
...
<pre class="bg-light"><code>make install
</code></pre>
...
<pre class="bg-light"><code>$ convert logo: logo.gif
$ identify logo.gif
</code></pre>
...
<pre class="bg-light"><code>rpmbuild --rebuild ImageMagick.src.rpm
</code></pre>
...
<pre class="bg-light"><code>$ sudo port -v install freetype +bytecode
...
<pre class="bg-light"><code>sudo port install jpeg
</code></pre>
...
<pre class="bg-light"><code>./configure --prefix=/opt --with-quantum-depth=16 \
...
<pre class="bg-light"><code>sudo make install
</code></pre>
  <p>To verify your install, type</p>

<pre class="bg-light"><code>/opt/local/bin/identify -list font
</code></pre>

  <p>to list all the fonts ImageMagick knows about.</p>
	  <p>To test the ImageMagick GUI, in a new shell, type:</p>

<pre class="bg-light"><code>display -display :0
</code></pre>
...
<pre class="bg-light"><code>configure LDFLAGS='-L/usr/local/lib -R/usr/local/lib'
</code></pre>
...
<pre class="bg-light"><code>/sbin/ldconfig /usr/local/lib
</code></pre>
...
<pre class="bg-light"><code>ldd `which convert`
</code></pre>
...
<pre class="bg-light"><code>yum install libjpeg libjpeg-devel
</code></pre>
