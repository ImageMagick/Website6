...
<p class="lead magick-description">ImageMagick uses an ASCII string known as <var>convert</var> (e.g. <samp>GIF</samp>) to identify file formats, algorithms acting as formats, built-in patterns, and embedded profile types.  Support for some of the formats are delegated to libraries or external programs. The Installation Guide describes where to find these distributions and any special configuration options required.</p>

<p>To get a complete listing of which image formats are supported on your system, type</p>

<pre class="bg-light text-dark mx-4"><samp>identify -list format
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>-> identify -verbose image.jpg
Image: image.jpg
...
<pre class="bg-light text-dark mx-4"><samp>$ convert image.jpg -print "%[colorspace]\n" null:
sRGB
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>convert image.jpg -colorspace RGB -resize 50% -colorspace sRGB resize.jpg
</samp></pre>
...
<pre class="bg-light text-dark mx-4"><samp>convert lab.tif -resize 50% resize.jpg
</samp></pre>
...
<table class="table table-sm table-hover table-striped table-responsive">
  ...
  <tr>
    ...
    <td>Requires the <a href="https://jxrlib.codeplex.com/">jxrlib</a> delegate library. Put the JxrDecApp and JxrEncApp applications in your execution path. Add the <samp>-define quantum:format=floating-point</samp> optionto minimize distortion.</td>
  </tr>
  ...
  <tr>
    ...
    <td>This format permits you to write to and read images from memory.  The filename is the registry key. The image persists until you explicitedly delete it or the program exits.  For example, let's use the MPR to create a checkerboard:
<pre class="bg-light text-dark mx-4"><samp>convert \( -size 15x15 canvas:black canvas:white -append \) \
  ...
  <tr>
    ...
    <td>Requires <a href="https://www.ghostscript.com/download.html">Ghostscript</a> to read.  By default, ImageMagick sets the page size to the MediaBox. Some PDF files, however, have a CropBox or TrimBox that is smaller than the MediaBox and may include white space, registration or cutting marks outside the CropBox or TrimBox. To force ImageMagick to use the CropBox or TrimBox rather than the MediaBox, use <a href="<?php echo $_SESSION['RelativePath']?>/../script/command-line-options.php#define">-define</a> (e.g. <samp>-define pdf:use-cropbox=true</samp> or <samp>-define pdf:use-trimbox=true</samp>).  Use <a href="<?php echo $_SESSION['RelativePath']?>/../script/command-line-options.php#density">-density</a> to improve the appearance of your PDF rendering (e.g. -density 300x300).  Use <a href="<?php echo $_SESSION['RelativePath']?>/../script/command-line-options.php#alpha">-alpha remove </a> to remove transparency. To specify direct conversion from  Postscript to PDF, use <samp>-define delegate:bimodel=true</samp>. Use <samp>-define pdf:fit-page=true</samp> to scale to the page size. To immediately stop processing upon an error, set <samp>-define pdf:stop-on-error</samp> to <samp>true</samp>. To set the page direction preferences to right-to-left, try  <samp>-define pdf:page-direction=right-to-left</samp>. Use <a href="<?php echo $_SESSION['RelativePath']?>/../script/command-line-options.php#alpha">-alpha remove </a> to remove transparency.  When writing to a PDF, thumbnails are included by default.  To skip generating thumbnails, <samp>-define pdf:thumbnail=false</samp>. To enable interpolation when rendering, use <samp>-define pdf:interpolate=true</samp>.</td>
  </tr>
  ...
  <tr>
    ...
    <td>Example usage: <samp>convert -density 300 pages?.pdf pocketmod:organize.pdf</samp> </td>
  </tr>
...
<table class="table table-sm table-hover table-striped table-responsive">
  ...
  <tr>
    ...
    <td>By default, a reasonable canvas size and border width is determined relative to the image collection you provide.  You can explicitedly set the canvas size and border width by appending to the filename, e.g. <samp>ashlar:canvas.png[1024x768+4+4]</samp>. By default, alignment is along the left edge.  Use <samp>-define ashlar:best-fit=true</samp> to align on both the left  and right edges.  You can label the image tiles with, for example, <samp>-label %f</samp>.</td>
  </tr>
