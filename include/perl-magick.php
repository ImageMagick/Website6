...
<pre class="bg-light text-dark mx-4"><code>tar xvzf ImageMagick.tar.gz
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>perl -MImage::Magick -le 'print Image::Magick->QuantumDepth'
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>cd PerlMagick
perl Makefile.nt
nmake
nmake install
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>make test
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>cd demo
make
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>use Image::Magick;
...
<pre class="bg-light text-dark mx-4"><code>use Image::Magick::Q16;
...
<pre class="bg-light text-dark mx-4"><code>$image = Image::Magick-&gt;new(size=&gt;'384x256');
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>$ENV{PATH}' . "='/../bin:/usr/bin:/usr/local/bin';
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>#!/usr/local/bin/perl
use Image::Magick;<br />
my($image, $x);<br />
$image = Image::Magick-&gt;new;
...
<pre class="pre-scrollable bg-light text-dark mx-4"><code>#!/usr/local/bin/perl
use Image::Magick;<br />
my($image, $p, $q);<br />
...
<pre class="bg-light text-dark mx-4"><code>$image = Image::Magick-&gt;new;
...
<pre class="bg-light text-dark mx-4"><code>@pixels = $image-&gt;GetPixel(x=&gt;1,y=&gt;1);
$pixels[0]*=0.5;
...
<pre class="bg-light text-dark mx-4"><code>$image-&gt;Quantize(colorspace=&gt;'gray');
</code></pre>

<p>Let's annotate an image with a Taipai TrueType font:</p>

<pre class="bg-light text-dark mx-4"><code>$text = 'Works like magick!';
$image-&gt;Annotate(font=&gt;'kai.ttf', pointsize=&gt;40, fill=&gt;'green', text=&gt;$text);
</code></pre>

<p>Perhaps you want to extract all the pixel intensities from an image and write them to STDOUT:</p>

<pre class="bg-light text-dark mx-4"><code>@pixels = $image-&gt;GetPixels(map=&gt;'I', height=&gt;$height, width=&gt;$width, normalize=&gt;true);
binmode STDOUT;
...
<pre class="bg-light text-dark mx-4"><code>$i = $#$p"+1";   # return the number of images associated with object p
...
<pre class="bg-light text-dark mx-4"><code>$image-&gt;Write(filename=&gt;'image.png', compression=&gt;'None');
</code></pre>

<p>Use <code>-</code> as the filename to method Read() to read from standard in or to method Write() to write to standard out:</p>

<pre class="bg-light text-dark mx-4"><code>binmode STDOUT;
$image-&gt;Write('png:-');
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>$filename = "image.png";
open(IMAGE, ">$filename");
...
<pre class="bg-light text-dark mx-4"><code>image%03d.miff
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>$image-&gt;Crop(geometry=&gt;'100x100+10+20');
...
<pre class="bg-light text-dark mx-4"><code>image-&gt;Draw(fill=&gt;'red', primitive=&gt;'rectangle',
 points=&gt;'20,20 100,100  40,40 200,200  60,60 300,300');
</code></pre>

<p>Is equivalent to</p>

<pre class="bg-light text-dark mx-4"><code>$image-&gt;Draw(fill=&gt;'red', primitive=&gt;'@draw.txt');
</code></pre>

<p>Where <code>draw.txt</code> is a file on disk that contains this:</p>

<pre class="bg-light text-dark mx-4"><code>rectangle 20, 20 100, 100
...
<pre class="pre-scrollable bg-light text-dark mx-4"><code>%b   file size
%c   comment
%d   directory
...
<pre class="bg-light text-dark mx-4"><code>text=&gt;"%m:%f %wx%h"
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>$image-&gt;Set(dither=&gt;'True');
$image-&gt;[$x]-&gt;Set(delay=&gt;3);
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>($a, $b, $c) = $image-&gt;Get('colorspace', 'magick', 'adjoin');
...
<pre class="bg-light text-dark mx-4"><code>$image-&gt;Montage(geometry=&gt;'160x160', tile=&gt;'2x2', texture=&gt;'granite:');
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>@blobs = $image-&gt;ImageToBlob();
...
<pre class="bg-light text-dark mx-4"><code>@blob=$db-&gt;GetImage();
...
<pre class="bg-light text-dark mx-4"><code>$p = $image-&gt;Append(stack=&gt;{true,false});
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>$q = $p-&gt;Clone();
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>$p = $images-&gt;Flatten(background=&gt;'none');
$p-&gt;Write('flatten.png');
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>$p = $image-&gt;Fx(expression=&gt;'(g+b)/2.0',channel=&gt;'red');
$p-&gt;Write('fx.miff');
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>$p = $image-&gt;Morph(frames=&gt;<i>integer</i>);
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>$image-&gt;Crop('340x256+0+0');
...
<pre class="bg-light text-dark mx-4"><code>$image-&gt;MogrifyRegion('100x100+40+50', 'modulate', brightness=&gt;50);
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>@phash = $image-&gt;PerceptualHash();</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>($width, $height, $size, $format) = $image-&gt;Ping('logo.png');
...
<pre class="bg-light text-dark mx-4"><code>$p = $image-&gt;Poly([0.5,1.0,0.25,2.0,1.0,1.0]);
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>$preview = $image-&gt;Preview('Gamma');
$preview-&gt;Display();
</code></pre>

<p>To have full control over text positioning you need font metric information. Use</p>

<pre class="bg-light text-dark mx-4"><code>($x_ppem, $y_ppem, $ascender, $descender, $width, $height, $max_advance) =
...
<pre class="bg-light text-dark mx-4"><code>@colors = $image-&gt;QueryColor();
...
<pre class="bg-light text-dark mx-4"><code>$name = $image-&gt;QueryColorname('rgba(80,60,0,0)');
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>@fonts = $image-&gt;QueryFont();
...
<pre class="bg-light text-dark mx-4"><code>@formats = $image-&gt;QueryFormat();
...
<pre class="bg-light text-dark mx-4"><code>$mime = $image-&gt;MagickToMime('tif');
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>$image-&gt;RemoteCommand('image.jpg');
</code></pre>

<p>The Smush() method smushes a set of images together. For example,</p>

<pre class="bg-light text-dark mx-4"><code>$p = $image-&gt;Smush(stack=&gt;{true,false},offset=&gt;<var>integer</var>);
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>@statistics = $image-&gt;Statistics();
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>$p = $image-&gt;Transform(crop=&gt;'100x100+0+0');
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>$x = $image-&gt;Read(...);
...
<pre class="bg-light text-dark mx-4"><code>$x = $image-&gt;Crop(...);
warn "$x" if "$x";      # print the error message
$x =~ /(\d+)/;
print $1;               # print the error number
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>$x = $image-&gt;Montage(...);
...
<pre class="bg-light text-dark mx-4"><code>Error 400: Memory allocation failed
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>$x = $image-&gt;Read('rose.png');
$x =~ /(\d+)/;
...
<pre class="pre-scrollable bg-light text-dark mx-4"><code>BlobError
BlobWarning
CacheError
CacheWarning
CoderError
...
<pre class="bg-light text-dark mx-4"><code>Image::Magick-&gt;QuantumDepth
</code></pre>
