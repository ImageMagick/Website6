...
<pre><code>yum install ImageMagick-perl
</code></pre>
...
<pre><code>perl -MImage::Magick -le 'print Image::Magick->QuantumDepth'
</code></pre>
...
<pre><code>$image = Image::Magick-&gt;new(size=&gt;'384x256');
</code></pre>
...
<pre><code>$ENV{PATH}' . "='/../bin:/usr/bin:/usr/local/bin';
</code></pre>
...
<pre><code>@pixels = $image-&gt;GetPixel(x=&gt;1,y=&gt;1);
$pixels[0]*=0.5;
...
<pre><code>$i = $#$p"+1";   # return the number of images associated with object p
...
<pre><code>$image-&gt;Write(filename=&gt;'image.png', compression=&gt;'None');
</code></pre>
...
<pre><code>$image-&gt;Crop(geometry=&gt;'100x100+10+20');
...
<pre><code>image-&gt;Draw(fill=&gt;'red', primitive=&gt;'rectangle',
 points=&gt;'20,20 100,100  40,40 200,200  60,60 300,300');
</code></pre>

<p>Is equivalent to</p>

<pre><code>$image-&gt;Draw(fill=&gt;'red', primitive=&gt;'@draw.txt');
</code></pre>

<p>Where <code>draw.txt</code> is a file on disk that contains this:</p>

<pre><code>rectangle 20, 20 100, 100
...
<pre><code>$image-&gt;Set(dither=&gt;'True');
$image-&gt;[$x]-&gt;Set(delay=&gt;3);
</code></pre>
...
<pre><code>($a, $b, $c) = $image-&gt;Get('colorspace', 'magick', 'adjoin');
...
<pre><code>$image-&gt;Montage(geometry=&gt;'160x160', tile=&gt;'2x2', texture=&gt;'granite:');
</code></pre>
...
<pre><code>@blobs = $image-&gt;ImageToBlob();
...
<pre><code>$p = $image-&gt;Append(stack=&gt;{true,false});
</code></pre>
...
<pre><code>@features = $image-&gt;Features(1);
</code></pre>
...
<pre><code>$p = $images-&gt;Transform(crop=>'100x100+10+60');
</code></pre>

<p>The Flatten() method flattens a set of images and returns it. For example,</p>

<pre><code>$p = $images-&gt;Flatten(background=&gt;'none');
$p-&gt;Write('flatten.png');
</code></pre>
...
<pre><code>$p = $image-&gt;Fx(expression=&gt;'(g+b)/2.0',channel=&gt;'red');
$p-&gt;Write('fx.miff');
</code></pre>
...
<pre><code>$p = $image-&gt;Morph(frames=&gt;<i>integer</i>);
</code></pre>
...
<pre><code>$image-&gt;Crop('340x256+0+0');
...
<pre><code>$image-&gt;MogrifyRegion('100x100+40+50', 'modulate', brightness=&gt;50);
</code></pre>
...
<pre><code>($width, $height, $size, $format) = $image-&gt;Ping('logo.png');
...
<pre><code>$p = $image-&gt;Poly([0.5,1.0,0.25,2.0,1.0,1.0]);
</code></pre>
...
<pre><code>$preview = $image-&gt;Preview('Gamma');
$preview-&gt;Display();
</code></pre>

<p>To have full control over text positioning you need font metric information. Use</p>

<pre><code>($x_ppem, $y_ppem, $ascender, $descender, $width, $height, $max_advance) =
...
<pre><code>@colors = $image-&gt;QueryColor();
...
<pre><code>$name = $image-&gt;QueryColorname('rgba(80,60,0,0)');
</code></pre>
...
<pre><code>@fonts = $image-&gt;QueryFont();
...
<pre><code>@formats = $image-&gt;QueryFormat();
...
<pre><code>$mime = $image-&gt;MagickToMime('tif');
</code></pre>
...
<pre><code>$image-&gt;RemoteCommand('image.jpg');
</code></pre>

<p>The Smush() method smushes a set of images together. For example,</p>

<pre><code>$p = $image-&gt;Smush(stack=&gt;{true,false},offset=&gt;<var>integer</var>);
</code></pre>
...
<pre><code>@statistics = $image-&gt;Statistics();
</code></pre>

<p>Finally, the Transform() method accepts a fully-qualified geometry specification for cropping or resizing one or more images.  For example,</p>

<pre><code>$p = $image-&gt;Transform(crop=&gt;'100x100+0+0');
</code></pre>
...
<pre><code>Error 400: Memory allocation failed
</code></pre>
...
<pre><code>$x = $image-&gt;Read('rose.png');
$x =~ /(\d+)/;
...
<pre><code>Image::Magick-&gt;QuantumDepth
</code></pre>
