...
<ul><pre class="bg-light"><code>yum install ImageMagick-perl
</code></pre></ul>
...
<ul><pre class="bg-light"><code>tar xvzf ImageMagick.tar.gz
</code></pre></ul>
...
<ul><pre class="bg-light"><code>perl -MImage::Magick -le 'print Image::Magick->QuantumDepth'
</code></pre></ul>
...
<ul><pre class="bg-light"><code>cd PerlMagick
perl Makefile.nt
nmake
nmake install
</code></pre></ul>
...
<ul><pre class="bg-light"><code>make test
</code></pre></ul>
...
<ul><pre class="bg-light"><code>cd demo
make
</code></pre></ul>
...
<ul><pre class="bg-light"><code>use Image::Magick;
...
<ul><pre class="bg-light"><code>use Image::Magick::Q16;
...
<ul><pre class="bg-light"><code>$image = Image::Magick-&gt;new(size=&gt;'384x256');
</code></pre></ul>
...
<ul><pre class="bg-light"><code>$ENV{PATH}' . "='/../bin:/usr/bin:/usr/local/bin';
</code></pre></ul>
...
<ul><pre class="bg-light"><code>#!/usr/local/bin/perl
use Image::Magick;<br />
my($image, $x);<br />
$image = Image::Magick-&gt;new;
...
<ul><pre class="bg-light"><code>$image = Image::Magick-&gt;new;
...
<ul><pre class="bg-light"><code>@pixels = $image-&gt;GetPixel(x=&gt;1,y=&gt;1);
$pixels[0]*=0.5;
...
<ul><pre class="bg-light"><code>$image-&gt;Quantize(colorspace=&gt;'gray');
</code></pre></ul>

<p>Let's annotate an image with a Taipai TrueType font:</p>

<ul><pre class="bg-light"><code>$text = 'Works like magick!';
$image-&gt;Annotate(font=&gt;'kai.ttf', pointsize=&gt;40, fill=&gt;'green', text=&gt;$text);
</code></pre></ul>

<p>Perhaps you want to extract all the pixel intensities from an image and write them to STDOUT:</p>

<ul><pre class="bg-light"><code>@pixels = $image-&gt;GetPixels(map=&gt;'I', height=&gt;$height, width=&gt;$width, normalize=&gt;true);
binmode STDOUT;
...
<ul><pre class="bg-light"><code>$i = $#$p"+1";   # return the number of images associated with object p
...
<ul><pre class="bg-light"><code>$filename = "image.png";
open(IMAGE, ">$filename");
...
<ul><pre class="bg-light"><code>image%03d.miff
</code></pre></ul>
...
<ul><pre class="bg-light"><code>$image-&gt;Crop(geometry=&gt;'100x100+10+20');
...
<ul><pre class="bg-light"><code>image-&gt;Draw(fill=&gt;'red', primitive=&gt;'rectangle',
 points=&gt;'20,20 100,100  40,40 200,200  60,60 300,300');
</code></pre></ul>

<p>Is equivalent to</p>

<ul><pre class="bg-light"><code>$image-&gt;Draw(fill=&gt;'red', primitive=&gt;'@draw.txt');
</code></pre></ul>

<p>Where <code>draw.txt</code> is a file on disk that contains this:</p>

<ul><pre class="bg-light"><code>rectangle 20, 20 100, 100
...
<ul><pre class="bg-light"><code>text=&gt;"%m:%f %wx%h"
</code></pre></ul>
...
<ul><pre class="bg-light"><code>$image-&gt;Set(dither=&gt;'True');
$image-&gt;[$x]-&gt;Set(delay=&gt;3);
</code></pre></ul>
...
<ul><pre class="bg-light"><code>($a, $b, $c) = $image-&gt;Get('colorspace', 'magick', 'adjoin');
...
<ul><pre class="bg-light"><code>$image-&gt;Montage(geometry=&gt;'160x160', tile=&gt;'2x2', texture=&gt;'granite:');
</code></pre></ul>
...
<ul><pre class="bg-light"><code>@blobs = $image-&gt;ImageToBlob();
...
<ul><pre class="bg-light"><code>@blob=$db-&gt;GetImage();
...
<ul><pre class="bg-light"><code>$p = $image-&gt;Append(stack=&gt;{true,false});
</code></pre></ul>
...
<ul><pre class="bg-light"><code>$q = $p-&gt;Clone();
</code></pre></ul>
...
<ul><pre class="bg-light"><code>@features = $image-&gt;Features(1);
</code></pre></ul>

<p>The Flatten() method flattens a set of images and returns it. For example,</p>

<ul><pre class="bg-light"><code>$p = $images-&gt;Flatten(background=&gt;'none');
$p-&gt;Write('flatten.png');
</code></pre></ul>
...
<ul><pre class="bg-light"><code>$p = $image-&gt;Fx(expression=&gt;'(g+b)/2.0',channel=&gt;'red');
$p-&gt;Write('fx.miff');
</code></pre></ul>
...
<ul><pre class="bg-light"><code>$p = $image-&gt;Morph(frames=&gt;<i>integer</i>);
</code></pre></ul>
...
<ul><pre class="bg-light"><code>$image-&gt;Crop('340x256+0+0');
...
<ul><pre class="bg-light"><code>$image-&gt;MogrifyRegion('100x100+40+50', 'modulate', brightness=&gt;50);
</code></pre></ul>
...
<ul><pre class="bg-light"><code>@phash = $image-&gt;PerceptualHash();</code></pre></ul>
...
<ul><pre class="bg-light"><code>($width, $height, $size, $format) = $image-&gt;Ping('logo.png');
...
<ul><pre class="bg-light"><code>$p = $image-&gt;Poly([0.5,1.0,0.25,2.0,1.0,1.0]);
</code></pre></ul>
...
<ul><pre class="bg-light"><code>$preview = $image-&gt;Preview('Gamma');
$preview-&gt;Display();
</code></pre></ul>

<p>To have full control over text positioning you need font metric information. Use</p>

<ul><pre class="bg-light"><code>($x_ppem, $y_ppem, $ascender, $descender, $width, $height, $max_advance) =
...
<ul><pre class="bg-light"><code>@colors = $image-&gt;QueryColor();
...
<ul><pre class="bg-light"><code>$name = $image-&gt;QueryColorname('rgba(80,60,0,0)');
</code></pre></ul>
...
<ul><pre class="bg-light"><code>@fonts = $image-&gt;QueryFont();
...
<ul><pre class="bg-light"><code>@formats = $image-&gt;QueryFormat();
...
<ul><pre class="bg-light"><code>$mime = $image-&gt;MagickToMime('tif');
</code></pre></ul>
...
<ul><pre class="bg-light"><code>$image-&gt;RemoteCommand('image.jpg');
</code></pre></ul>

<p>The Smush() method smushes a set of images together. For example,</p>

<ul><pre class="bg-light"><code>$p = $image-&gt;Smush(stack=&gt;{true,false},offset=&gt;<var>integer</var>);
</code></pre></ul>
...
<ul><pre class="bg-light"><code>@statistics = $image-&gt;Statistics();
</code></pre></ul>
...
<ul><pre class="bg-light"><code>$p = $image-&gt;Transform(crop=&gt;'100x100+0+0');
</code></pre></ul>
...
<ul><pre class="bg-light"><code>$x = $image-&gt;Read(...);
...
<ul><pre class="bg-light"><code>$x = $image-&gt;Crop(...);
warn "$x" if "$x";      # print the error message
$x =~ /(\d+)/;
print $1;               # print the error number
</code></pre></ul>
...
<ul><pre class="bg-light"><code>$x = $image-&gt;Montage(...);
...
<ul><pre class="bg-light"><code>Error 400: Memory allocation failed
</code></pre></ul>
...
<ul><pre class="bg-light"><code>$x = $image-&gt;Read('rose.png');
$x =~ /(\d+)/;
...
<ul><pre class="bg-light"><code>Image::Magick-&gt;QuantumDepth
</code></pre></ul>
