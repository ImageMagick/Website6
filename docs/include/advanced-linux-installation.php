...
cd ImageMagick-<?php echo MagickVersion; ?>
...
<pre class="pre-scrollable bg-light text-dark mx-4"><code>ImageMagick is configured as follows. Please verify that this configuration
...
<pre class="bg-light text-dark mx-4"><code>configure CC=c99 CFLAGS=-O2 LDFLAGS='-L/usr/local/lib -R/usr/local/lib' LIBS=-lposix
</code></pre>
...
<table class="table table-sm table-hover table-striped table-responsive">
  ...
  <br /><br />
<pre class="bg-light text-dark mx-4">Quantum Depth     Valid Range (Decimal)   Valid Range (Hex)
    8             0-255                   00-FF
  ...
  <br /><br />
<pre class="bg-light text-dark mx-4">Quantum Depth   Virtual Memory
     8               3MB
    16               8MB
    32              15MB
</pre></td>
  ...
  <br /><br />
  By default, PerlMagick is conveniently compiled and installed as part of ImageMagick's normal <code>configure</code>, <code>make</code>, <code>sudo make install</code> process. When <code>--without-perl</code> is specified, you must first install ImageMagick, download the <a href="https://imagemagick.org/archive/perl/">PerlMagick</a> source distribution, build, and finally install PerlMagick. Note, PerlMagick is configured even if <code>--without-perl</code> is specified. If the argument <var>--with-perl=/path/to/perl</var> is supplied, <var>/../path/to/perl</var> is be taken as the PERL interpreter to use. This important in case the <code>perl</code> executable in your PATH is not PERL5, or is not the PERL you want to use.</td>
  </tr>
  <tr>
...
<p>If all reasonable corrective actions have been tried and the problem appears be due to a flaw in the configure script, please send a bug report to the <a href="https://github.com/ImageMagick/ImageMagick6/issues">ImageMagick Defect Support Forum</a>. All bug reports should contain the operating system type (as reported by <code>uname -a</code>) and the compiler/compiler-version. A copy of the configure script output and/or the relevant portion of  <code>config.log</code> file may be valuable in order to find the problem.  If you post portions of <code>config.log</code>, please also send a script of the configure output and a description of what you expected to see (and why) so the failure you are observing can be identified and resolved.</p>
...
<pre class="bg-light text-dark mx-4"><code>make
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>make install
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>identify -list configure
</code></pre>

<p>To list which image formats are supported , type:</p>

<pre class="bg-light text-dark mx-4"><code>identify -list format
</code></pre>
...
<p>Download <a href="https://imagemagick.org/archive//linux/SRPMS/ImageMagick.src.rpm">ImageMagick.src.rpm</a> from <a href="https://imagemagick.org/archive">imagemagick.org</a> or its <a href="<?php echo $_SESSION['RelativePath']?>/../script/mirror.php">mirrors</a>.</p>

<p>Build ImageMagick with this command:</p>

<pre class="bg-light text-dark mx-4"><code>rpmbuild --rebuild ImageMagick.src.rpm
</code></pre>

<p>After the build you, locate the RPMS folder and install the ImageMagick binary RPM distribution:</p>

<pre class="p-3 mb-2 text-body-secondary bg-body-tertiary"><samp>rpm -ivh ImageMagick-<?php echo MagickLibVersion ?>.-?.*.rpm</samp></pre>
...
<pre class="bg-light text-dark mx-4"><code>$ sudo port -v install freetype +bytecode
...
<pre class="bg-light text-dark mx-4"><code>sudo port install jpeg
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>/opt/local/bin/identify -list font
</code></pre>

  <p>to list all the fonts ImageMagick knows about.</p>
	  <p>To test the ImageMagick GUI, in a new shell, type:</p>

<pre class="bg-light text-dark mx-4"><code>display -display :0
</code></pre>
...
<pre class="p-3 mb-2 text-body-secondary bg-body-tertiary"><samp>tar jxvf ImageMagick-<?php echo MagickLibVersion ?>-?.tar.bz2
cd ImageMagick-<?php echo MagickVersion ?>
...
<pre class="bg-light text-dark mx-4"><code>configure LDFLAGS='-L/usr/local/lib -R/usr/local/lib'
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>/sbin/ldconfig /usr/local/lib
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>ldd `which convert`
</code></pre>
...
<pre class="bg-light text-dark mx-4"><code>yum install libjpeg libjpeg-devel
</code></pre>
